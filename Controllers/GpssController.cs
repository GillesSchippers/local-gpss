...
namespace Controllers
{
    ...
    public class GpssController : ControllerBase
    {
        ...
        public IActionResult Upload([FromRoute] string entityType)
        {
            ...
            List<string> generations = [.. Strings.Split(generationsStr, ",")];
            List<long> ids = [];
            ...
            [FromQuery] bool download = false)
        {
            ...
            Database.Instance.IncrementDownload(
            ...
            if (entityType == "pokemon")
            {
                var pokemon = Database.Instance.List<GpssPokemon>("pokemon", search: new Search { DownloadCode = code }).FirstOrDefault();
                if (string.IsNullOrEmpty(pokemon.DownloadCode))
                    return NotFound(new { message = "Pokemon not found." });
                return Ok(pokemon);
            }
            else // bundle or bundles
            {
                var bundle = Database.Instance.List<GpssBundle>("bundle", search: new Search { DownloadCode = code }).FirstOrDefault();
